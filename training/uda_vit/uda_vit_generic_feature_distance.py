...
class UDAVITGenericFeatureDistance(lightning.LightningModule):
    def __init__(
            self,
            batch_size: int,
            img_size: int,
    ...
    def train_dataprep(self, batch):
        ...
        source_frozen_feats = self.network_pretrained_frozen.forward_features(
            normalize(
                F.interpolate(
                    sourceds_aug_image,
                    size=(self.network_pretrained_frozen.vit_in_img_size,
                          self.network_pretrained_frozen.vit_in_img_size),
                    mode="bilinear", align_corners=self.network_pretrained_frozen.align_corners
                )
            ),
            0
        )
    ...
    def training_step(
        ...
        # supervised training
        ##
        source_logits, source_feats = self.network(normalize(sourceds_aug_image), return_features=True)
        loss_source = F.cross_entropy(source_logits, sourceds_aug_target, ignore_index=self.ignore_index)

        ##
        # feature distance
        ##
        loss_fdist = self.get_feature_distance_loss(source_feats, source_frozen_feats, sourceds_aug_target)

        self.manual_backward(loss_source + loss_fdist)
        loss_source = loss_source.detach()
        source_logits = source_logits.detach()
        loss_fdist = loss_fdist.detach()
    ...
    def get_feature_distance_loss(self, feats, feats_frozen, segmentation_label):
        ...
        # get mask
        ##
        segmentation_label = segmentation_label.detach().clone().unsqueeze(1)  # convert label to bchw format
        segmentation_label = F.interpolate(
            segmentation_label.to(torch.uint8),
            size=(self.network_pretrained_frozen.vit_in_img_size,
                  self.network_pretrained_frozen.vit_in_img_size),
            mode="nearest"
        ).long()
        ...
        assert self.network_pretrained_frozen.patch_size == scale_factor
    ...
    def get_pseudo_label_and_weight(self, targetds_image, targetds_ignore_mask):
        ...
        ema_logit = self.network_ema(targetds_image)
