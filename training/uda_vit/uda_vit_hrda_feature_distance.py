...
from models.daformer.utils import downscale_label_ratio
...
from training.utils.get_hrda_head_loss import get_hrda_head_loss
...
class UDAVITHRDAFeatureDistance(lightning.LightningModule):
    def __init__(
            self,
            batch_size: int,
            img_size: int,
        ...
        # uda specific params
        ##
        self.hr_loss_weight = 0.1
        self.ema_alpha = 0.999
        self.color_jitter_strength = 0.2
        self.color_jitter_probability = 0.8
        self.pseudo_threshold = 0.968
        self.imnet_feature_dist_lambda = 0.005
        self.register_buffer("imnet_feature_dist_classes", torch.tensor([6, 7, 11, 12, 13, 14, 15, 16, 17, 18]))
        self.imnet_feature_dist_scale_min_ratio = 0.75
        ...
        self.random_aug_uda = random_aug_uda  # TV.Lambda(lambda x: torch.stack([random_aug_uda(x_) for x_ in x]))
        ...
        self.network_pretrained_frozen = copy.deepcopy(network)
        self.network_pretrained_frozen.load_state_dict(self.network.state_dict())
        for p in self.network_pretrained_frozen.parameters():
            p.requires_grad = False
    ...
    def train_dataprep(self, batch):
        ...
        source_frozen_feats = self.network_pretrained_frozen.forward_features(
            normalize(
                sourceds_image
            )
        )
        ...
        return (
            ...
            (pseudo_label, pseudo_weight, uncertainty, source_frozen_feats),
        )
    ...
    def training_step(
        ...
        pseudo_label, pseudo_weight, uncertainty, source_frozen_feats = other_stuff

        ##
        # supervised training
        ##
        sourceds_fused_seg, sourceds_seg_high, sourceds_crop_data, sourceds_embed_low = self.network(
            normalize(sourceds_aug_image))
        loss_source = get_hrda_head_loss(
            sourceds_fused_seg, sourceds_seg_high, sourceds_aug_target, sourceds_crop_data, self.hr_loss_weight,
            ignore_index=self.ignore_index
        )

        self.manual_backward(loss_source)
        loss_source = loss_source.detach()
        source_logits = sourceds_fused_seg.detach()

        ##
        # feature distance
        ##
        loss_fdist = self.get_feature_distance_loss(sourceds_embed_low, source_frozen_feats, sourceds_aug_target)

        ##
        # uda training
        ##
        mixed_fused_seg, mixed_seg_high, mixed_crop_data, _ = self.network(
            normalize(mixed_img))
        assert mixed_lbl.shape[1] == 1

        loss_mix = get_hrda_head_loss(
            mixed_fused_seg, mixed_seg_high, mixed_lbl[:, 0, :, :], mixed_crop_data, self.hr_loss_weight,
            label_weight=mixed_seg_weight, ignore_index=self.ignore_index
        )

        self.manual_backward(loss_mix)
        loss_mix = loss_mix.detach()
        mixed_logits = mixed_fused_seg.detach()

        if 0.0 < self.token_mask_ratio:
            targetds_color_aug_image = targetds_image.clone().detach()
            for bs_i in range(targetds_image.shape[0]):
                targetds_color_aug_image[bs_i] = self.random_aug_uda(targetds_color_aug_image[bs_i])

            masked_fused_seg, masked_seg_high, masked_crop_data, _ = self.network(
                normalize(targetds_color_aug_image), mask_ratio=self.token_mask_ratio)

            loss_masked = get_hrda_head_loss(
                masked_fused_seg, masked_seg_high, pseudo_label, masked_crop_data, self.hr_loss_weight,
                label_weight=pseudo_weight, ignore_index=self.ignore_index
            )
        ...
        self.log("train_loss_fdist", loss_fdist, prog_bar=True)
    ...
    def get_feature_distance_loss(self, feats, feats_frozen, segmentation_label):
        segmentation_label = segmentation_label.detach().clone().unsqueeze(1)
        scale_factor = segmentation_label.shape[-1] // feats.shape[-1]
        gt_rescaled = downscale_label_ratio(segmentation_label, scale_factor,
                                            self.imnet_feature_dist_scale_min_ratio,
                                            self.num_classes,
                                            255).long().detach()
        thing_stuff_mask = torch.any(gt_rescaled[..., None] == self.imnet_feature_dist_classes, -1)
        thing_stuff_mask = thing_stuff_mask.contiguous()
        #
        feat_dist = torch.norm(feats - feats_frozen, dim=1, p=2)
        feat_dist = feat_dist[thing_stuff_mask.squeeze(1)]
        return feat_dist.mean() * self.imnet_feature_dist_lambda
    ...
    def get_pseudo_label_and_weight(self, targetds_image, targetds_ignore_mask):
        ...
        ema_logit = self.network_ema.inference(targetds_image)
        ema_softmax = torch.softmax(ema_logit, dim=1)
    ...
    def eval_step(
        ...
        for crop, preds, count_mat, coords, target in zip(b_crop, b_preds, b_count_mat, b_coords, b_target):
            ...
            segmentation = slide_inference_pre_cropped(self.network.inference, crop, preds, count_mat, coords,
                                                       self.num_classes)
