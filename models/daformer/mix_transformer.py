...
def get_mit_backbone(model_name, pretrained=True):
    models = {
        "mit_b0": mit_b0,
        "mit_b1": mit_b1,
        "mit_b2": mit_b2,
    ...
    # download checkpoints from https://github.com/NVlabs/SegFormer
    chkpt_state_dict = torch.load("pretrained/{}.pth".format(model_name))
    # remove head
    chkpt_state_dict = {k: v for k, v in chkpt_state_dict.items() if "head" not in k}
    model.load_state_dict(chkpt_state_dict)
    return model
